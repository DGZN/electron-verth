...
var setInterface = function(_interface){
  iface = _interface;
  session = pcap.createSession(_interface, "");
  return;
}
...
var send = function(pkt_o){
  ...
  require('getmac').getMac(function(err, macAddr){
    ...
    pkt.dst = mac_to_arr("10:a5:d0:49:ac:cc"); // Broadcast Addrress: ff:ff:ff:ff:ff:ff

    pkt.src = mac_to_arr(pkt_o.src_mac);
